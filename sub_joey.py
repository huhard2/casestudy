...
def add_vxlan(interface,vxlan):
	test12 = subprocess.call(['sudo', 'ovs-vsctl','add-port', interface, vxlan])

def add_route(ip_preflix,bridge,gateway):
	test13 = subprocess.call(['sudo','ovs-appctl','ovs/route/add', ip_preflix , bridge , gateway ])

def show_route():
	test14 = subprocess.call(['sudo','ovs-appctl','ovs/route/show'])
	return test10

def del_route(ip_preflix):
	test15 = subprocess.call(['sudo','ovs-appctl' ,'ovs/route/del' , ip_preflix])
